...
def generate_hotel_data(n=1000, include_neutral=False):
    """Genera un CSV con recensioni sintetiche.


    reparti = ['Housekeeping', 'Reception', 'Food & Beverage']
    # sentiment binario per default, neutralità opzionale
    if include_neutral:
...
# generiamo titolo e corpo separati per maggiore realismo
        if sent == 'Positivo':
            title_templates = [
    ...
    # salviamo con le nuove colonne richieste
    df.to_csv('hotel_reviews.csv', index=False, columns=['review_id','date','title','body','department','sentiment'])
    print(f"✅ DATASET POTENZIATO: Generato con {n} recensioni ricche in 'hotel_reviews.csv'")
