...
def main(csv_path="hotel_reviews.csv"):
    ...
    # Ora il CSV contiene title e body separati
    df = df.dropna(subset=["title", "body", "department", "sentiment"])

    # Rimuoviamo le recensioni neutrali perché il modello sentimentale è binario
    before = len(df)
    df = df[df["sentiment"].isin(["Positivo", "Negativo"])]
    dropped = before - len(df)
    if dropped > 0:
        print(f"Nota: {dropped} righe con sentiment 'Neutro' eliminate (solo binario).\n")
